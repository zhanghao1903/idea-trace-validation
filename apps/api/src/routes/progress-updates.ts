...
import {
  ProgressCollectionRouteSchema,
  ProgressUpdateRouteSchema,
} from "@idea/contracts";
...
export const progressUpdateRoutes =
  (
  ...
  async (app) => {
    app.post(
      ...
      {
        schema: ProgressUpdateRouteSchema,
        preHandler: authenticateWrite(aiApiToken),
      },
      async (request, reply) => {
        const result = await service.createProgressUpdate(
    ...
    app.get(
      "/projects/:projectId/progress-updates",
      { schema: ProgressCollectionRouteSchema },
      async (request) => {
        const page = await service.listProgressUpdates(
          request.params.projectId,
