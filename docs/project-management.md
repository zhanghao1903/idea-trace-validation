...
| LP-05 | [部署与发布就绪](./implementation-plans/v0-1/lp-05-deployment-release.md) | LP-04 | Ready for Acceptance | 缺少绑定精确候选的服务器、域名/DNS、秘密/备份位置与生产部署授权 | 完成 Engineering Review；获独立目标授权后才执行真实部署、备份恢复和公网 re-smoke |
...
总体下一步：完成 LP-05 exact-head 全量门禁和 Engineering Review。代码合并后仍需独立
