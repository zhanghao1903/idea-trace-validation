...
| `RollbackCleanupEvidenceV1` / aggregate and scope-result references | authority refs, strict application-rollback digest, production/restore cleanup results, times/status/reason/digest; fixed attempt-relative references | Stored only as `<attempt>/rollback-cleanup.json`; resolvers reject alternate paths and recompute the digest. `ROLLBACK_CLEANUP` binds aggregate status and is the sole terminal-envelope authority. `ROLLBACK_CLEANUP_RESULT` binds the same immutable record/digest but its status to one selected scope result, preserving `CLEANED -> PASS` and `CLEANUP_FAILED -> FAIL` for mixed outcomes. Replay preserves original times and accepts only identical application and scoped results. Production PASS also requires the configured `{databaseUser,databaseName}`, zero application rows, exact deletion arguments and consecutive zero observations. Upgrade production is `NOT_APPLICABLE` and byte-preserved. |
