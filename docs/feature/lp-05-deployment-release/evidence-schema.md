...
| `AttemptRuntimeBindingV2` | `schemaVersion`, `attemptId`, `runtime`, `productionDatabase`, `previousEnvironmentSha256`, `bindingSha256` | Written before the first attempt mutation and byte-equal on restart. `productionDatabase` is the closed validated `{databaseUser,databaseName}` authority. Controller and manual recovery reject current principal drift before an application callback, database read, Docker observation/deletion or terminal write. V1 remains historical evidence only. |
...
| `RollbackCleanupEvidenceV1` / aggregate and scope-result references | authority refs, complete closed application rollback plus its digest, production/restore cleanup results, times/status/reason/digest; fixed attempt-relative references | Stored only as `<attempt>/rollback-cleanup.json`; resolvers reject alternate paths and recompute both application and aggregate digests. `ROLLBACK_CLEANUP` binds aggregate status and is the sole terminal-envelope authority. `ROLLBACK_CLEANUP_RESULT` binds the same immutable record/digest but its status to one selected scope result, preserving `CLEANED -> PASS` and `CLEANUP_FAILED -> FAIL` for mixed outcomes. Recovery resolves the aggregate before application rollback or cleanup, preserves its bytes and repeats no mutation. The production project is derived from the immutable attempt target and must equal both the aggregate and any execution argument before callbacks or Docker reads. Before completing missing lifecycle/terminal records recovery re-observes every PASS or NOT_APPLICABLE scope: cleaned/never-created projects remain empty and upgrade production retains the exact persisted resource-set digest. Reappearance, redirected configuration or set drift fails before terminalization and never authorizes another delete. Production PASS also requires the configured `{databaseUser,databaseName}`, zero application rows, exact deletion arguments and consecutive zero observations. |
| `ForwardRestoreCleanupEvidenceV1` / forward cleanup reference | authority refs, restore project/database, ordered times, one closed restore cleanup result, status/reason/digest; fixed attempt-relative reference | Stored only as `<attempt>/forward-restore-cleanup.json`. Normal restore cleanup must persist this record and transition the exact V2 lifecycle to `CLEANED` before returning PASS. If deletion completed before this record was written, only exact RESTORE `QUIESCING` authority with a nonempty frozen set plus consecutive empty live observations may recover without a second delete; partial/nonempty drift and production cleanup fail closed. Actual rollback preflights both pending and terminal lifecycle authority before application rollback or production cleanup: absence permits lifecycle-only recovery, exact matching content is resolved and reused, and malformed, wrong-bound, digest-conflicting or terminal-state-conflicting authority aborts with no new deletion or PASS evidence. Terminal PASS also requires the live project to remain empty; only the rollback aggregate may authorize terminal rollback evidence. |
| `TerminalRollbackEvidenceV1` | authority refs, strict application-rollback digest, cleanup reference/digest, terminal state/reason/evidence digest | `ROLLED_BACK` requires application rollback `PASS|NOT_APPLICABLE` and both independently resolved cleanup results to pass or be validly not applicable. Its verified state and digest drive both controller and manual rollback terminal transitions; an already-terminal manual replay must resolve the aggregate and reproduce the transition digest. Cleanup fields never enter the closed seven-field `DeploymentAttemptV1.rollback`, so cleanup failure may journal `ROLLBACK_FAILED` while preserving a non-failing application projection. |
