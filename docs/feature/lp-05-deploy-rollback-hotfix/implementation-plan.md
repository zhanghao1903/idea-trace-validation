...
| --- | --- | --- | --- | --- |
| S1 configured principal | `scripts/lp05/database/production-runtime.ts`; `database-principal.ts`; `runtime-binding.ts`; call sites in `controller-cli.ts` and `host-active-operations.ts`; tests | Add required `databaseUser`; both identity commands pass exact `--username` and `--dbname`; persist the validated user/name before mutation and reject restart drift | command-shape, wrong-user/no-fallback and binding tests; real PostgreSQL 17.10 bound-data/empty-decoy proof | AttemptRuntimeBinding V2 for new attempts; V1 remains historical |
...
| S3 rollback separation | `active-oracles.ts`; `host-active-operations.ts`; `controller.ts`; `manual-rollback.ts`; `rollback-cli.ts`; tests | Return strict application rollback separately from cleanup reference; project only existing rollback fields; route controller and manual recovery through the same aggregate/reconciliation/terminal authority; keep persisted recovery available after forward expiry and live-reconcile terminal replay | prior extra-key regression; successful `NOT_APPLICABLE` + cleanup PASS; cleanup FAIL; expired FAILED/ROLLING_BACK aggregate recovery; terminal production/restore reappearance and preserved-upgrade drift | Preserve seven-field rollback schema and attempt state graph; exact envelope identity/digest remains mandatory |
...
1. `inspectLiveProductionDatabaseIdentity` and `inspectLiveProductionIdentity` require `databaseUser` and pass it as
   a discrete Docker exec argument after `--username`. New attempts persist the validated user/name in
   `AttemptRuntimeBindingV2`; recovery uses only that binding and rejects current configuration drift.
