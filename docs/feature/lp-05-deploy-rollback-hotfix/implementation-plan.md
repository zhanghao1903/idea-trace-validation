...
| S3 rollback separation | `active-oracles.ts`; `host-active-operations.ts`; `controller.ts`; `manual-rollback.ts`; `rollback-cli.ts`; tests | Return strict application rollback separately from cleanup reference; project only existing rollback fields; route controller and manual recovery through the same aggregate/reconciliation/terminal authority | prior extra-key regression; successful `NOT_APPLICABLE` + cleanup PASS; cleanup FAIL; aggregate-write crash and terminal replay through manual entrypoint | Preserve seven-field rollback schema and attempt state graph |
